$(function () {
    ...
    PageClick = function (pageclickednumber) {
        ...
        param = { name: name, isvalid: isvalid, pagenumber: pageclickednumber, pagesize: pagesize };

        InvokeAction.InvokeJsonAction('MrpbasicMgr', 'ListUnit', param, function (jsonData) {
            ...
            if (jsonData != null && jsonData.rows != null && jsonData.rows.length > 0) {
                for (i = 0 ; i < jsonData.rows.length; i++) {
                    ...
                    htmllist += '<td><input type = "checkbox" name ="chkSelect"  value="' + jsonData.rows[i].Id + '" /></td>' +
                                    '<td>' + jsonData.rows[i].UnitCode + '</td>' +
                                    '<td>' + jsonData.rows[i].UnitName + '</td>' +
    ...
    $("#btnAdd").click(function () {
        var retval = winModalFullScreen("UnitForm", false, 800, 600);
        PageClick(1);
    });
    ...
    $("#btnModify").click(function () {
        if (CheckBoxOnly("chkSelect")) {
            ...
            var retval = winModalFullScreen("UnitForm?key=" + checkenodevalue, false, 800, 600);
            PageClick(1);
        }
    });
    ...
    $("#btnValid").click(function () {
        if (CheckBoxOnly("chkSelect")) {
            ...
            InvokeAction.InvokeJsonAction('MrpbasicMgr', 'UpdateUnitValid', param, function (jsonData) {
                PageClick(1);
                alert(jsonData);
    ...
    $("#btnRemove").click(function () {
        ...
        if (CheckBoxOnly("chkSelect")) {
            if (confirm("确定要删除该信息吗?")) {
                ...
                InvokeAction.InvokeJsonAction('MrpbasicMgr', 'DeleteUnit', param, function (jsonData) {
                    PageClick(1);
                    alert(jsonData);
                });
            }
