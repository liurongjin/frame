$(function () {
    ...
    PageClick = function (pageclickednumber) {
        ...
        var parentid = $("#drplProductOrder").val();
        ...
        InvokeAction.InvokeJsonAction('ProductOrderMgr', 'ListProductOrder', param, function (jsonData) {
            ...
            if (jsonData != null && jsonData.rows != null && jsonData.rows.length > 0) {
                for (i = 0 ; i < jsonData.rows.length; i++) {
                    ...
                    htmllist += '<td><input type = "checkbox" name ="chkSelect"  value="' + jsonData.rows[i].Id + '" /></td>' +
                                    '<td>' + jsonData.rows[i].ProductOrderCode + '</td>' +
                                    '<td>' + jsonData.rows[i].ProductOrderName + '</td>' +
                                    '<td>' + jsonData.rows[i].ProductOrderCatalogName + '</td>' +
    ...
    $("#btnAdd").click(function () {
        var retval = winModalFullScreen("ProductOrderForm", false, 800, 600);
        PageClick(1);
    });
    ...
    $("#btnModify").click(function () {
        if (CheckBoxOnly("chkSelect")) {
            ...
            var retval = winModalFullScreen("ProductOrderForm?key=" + checkenodevalue, false, 800, 600);
            PageClick(1);
        }
    });
    ...
    $("#btnValid").click(function () {
        if (CheckBoxOnly("chkSelect")) {
            ...
            InvokeAction.InvokeJsonAction('ProductOrderMgr', 'UpdateProductOrderValid', param, function (jsonData) {
                PageClick(1);
                alert(jsonData);
    ...
    $("#btnRemove").click(function () {
        ...
        if (CheckBoxOnly("chkSelect")) {
            if (confirm("确定要删除该信息吗?")) {
                ...
                InvokeAction.InvokeJsonAction('ProductOrderMgr', 'DeleteProductOrder', param, function (jsonData) {
                    PageClick(1);
                    alert(jsonData);
                });
            }
