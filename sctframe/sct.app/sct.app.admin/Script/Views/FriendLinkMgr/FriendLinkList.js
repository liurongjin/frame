$(function () {
    ...
    PageClick = function (pageclickednumber) {
        var title = $("#txtQName").val();
        var language = $("#drplLanguage").val();
        var friendtype = $("#drplFriendType").val();
        var isvalid = $("#drplIsvalid").val();
        param = { title: title, language: language, friendtype: friendtype, isvalid: isvalid, pagenumber: pageclickednumber, pagesize: pagesize };

        InvokeAction.InvokeJsonAction('FriendLinkMgr', 'ListFriendLink', param, function (jsonData) {
            ...
            if (jsonData != null && jsonData.rows != null && jsonData.rows.length > 0) {
                for (i = 0 ; i < jsonData.rows.length; i++) {
                    ...
                    htmllist += '<td><input type = "checkbox" name ="chkSelect"  value="' + jsonData.rows[i].Id + '" /></td>' +
                                    ...
                                    '<td>' + jsonData.rows[i].Language + '</td>' +
                                    '<td>' + jsonData.rows[i].LinkUrl + '</td>' +
                                    '<td>' + jsonData.rows[i].FriendType + '</td>' +
    ...
    $("#btnAdd").click(function () {
        var retval = winModalFullScreen("FriendLinkForm", false, 800, 600);
        PageClick(1);
    });
    ...
    $("#btnModify").click(function () {
        if (CheckBoxOnly("chkSelect")) {
            ...
            var retval = winModalFullScreen("FriendLinkForm?key=" + checkenodevalue, false, 800, 600);
            PageClick(1);
        }
    });
    ...
    $("#btnValid").click(function () {
        if (CheckBoxOnly("chkSelect")) {
            ...
            InvokeAction.InvokeJsonAction('FriendLinkMgr', 'UpdateFriendLinkValid', param, function (jsonData) {
                PageClick(1);
                alert(jsonData);
    ...
    $("#btnRemove").click(function () {
        ...
        if (CheckBoxOnly("chkSelect")) {
            if (confirm("确定要删除该信息吗?")) {
                ...
                InvokeAction.InvokeJsonAction('FriendLinkMgr', 'DeleteFriendLink', param, function (jsonData) {
                    PageClick(1);
                    alert(jsonData);
                });
            }
