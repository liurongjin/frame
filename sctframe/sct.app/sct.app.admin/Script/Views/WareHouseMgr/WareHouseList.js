$(function () {
    ...
    PageClick = function (pageclickednumber) {
        ...
        var regionid = $("#drplRegion").val();
        var isvalid = $("#drplIsvalid").val();
        param = { name: name, regionid: regionid, isvalid: isvalid, pagenumber: pageclickednumber, pagesize: pagesize };

        InvokeAction.InvokeJsonAction('WareHouseMgr', 'ListWareHouse', param, function (jsonData) {
            ...
            if (jsonData != null && jsonData.rows != null && jsonData.rows.length > 0) {
                for (i = 0 ; i < jsonData.rows.length; i++) {
                    ...
                    htmllist += '<td><input type = "checkbox" name ="chkSelect"  value="' + jsonData.rows[i].Id + '" /></td>' +
                                    '<td>' + jsonData.rows[i].WareHouseCode + '</td>' +
                                    '<td>' + jsonData.rows[i].WareHouseName + '</td>' +
                                    '<td>' + jsonData.rows[i].RegionId + '</td>' +
                                    '<td>' + jsonData.rows[i].Address + '</td>' +
    ...
    $("#btnAdd").click(function () {
        var retval = winModalFullScreen("WareHouseForm", false, 800, 600);
        PageClick(1);
    });
    ...
    $("#btnModify").click(function () {
        if (CheckBoxOnly("chkSelect")) {
            ...
            var retval = winModalFullScreen("WareHouseForm?key=" + checkenodevalue, false, 800, 600);
            PageClick(1);
        }
    });
    ...
    $("#btnValid").click(function () {
        if (CheckBoxOnly("chkSelect")) {
            ...
            InvokeAction.InvokeJsonAction('WareHouseMgr', 'UpdateWareHouseValid', param, function (jsonData) {
                PageClick(1);
                alert(jsonData);
    ...
    $("#btnRemove").click(function () {
        ...
        if (CheckBoxOnly("chkSelect")) {
            if (confirm("确定要删除该信息吗?")) {
                ...
                InvokeAction.InvokeJsonAction('WareHouseMgr', 'DeleteWareHouse', param, function (jsonData) {
                    PageClick(1);
                    alert(jsonData);
                });
            }
