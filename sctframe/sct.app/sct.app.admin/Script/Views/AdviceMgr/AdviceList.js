$(function () {
    ...
    PageClick = function (pageclickednumber) {
        ...
        var state = $("#drplState").val();
        var isvalid = $("#drplIsvalid").val();
        param = { title: title, state: state, language: language, isvalid: isvalid, pagenumber: pageclickednumber, pagesize: pagesize };

        InvokeAction.InvokeJsonAction('AdviceMgr', 'ListAdvice', param, function (jsonData) {
            ...
            if (jsonData != null && jsonData.rows != null && jsonData.rows.length > 0) {
                for (i = 0 ; i < jsonData.rows.length; i++) {
                    ...
                    htmllist += '<td><input type = "checkbox" name ="chkSelect"  value="' + jsonData.rows[i].Id + '" /></td>' +
                                    ...
                                    '<td>' + jsonData.rows[i].Contact + '</td>' +
                                    '<td>' + jsonData.rows[i].ContactMethod + '</td>' +
                                    '<td>' + jsonData.rows[i].State + '</td>' +
    ...
    $("#btnAdd").click(function () {
        var retval = winModalFullScreen("AdviceForm", false, 800, 600);
        PageClick(1);
    });
    ...
    $("#btnModify").click(function () {
        if (CheckBoxOnly("chkSelect")) {
            ...
            var retval = winModalFullScreen("AdviceForm?key=" + checkenodevalue, false, 800, 600);
            PageClick(1);
        }
    });
    ...
    $("#btnValid").click(function () {
        if (CheckBoxOnly("chkSelect")) {
            ...
            InvokeAction.InvokeJsonAction('AdviceMgr', 'UpdateAdviceValid', param, function (jsonData) {
                PageClick(1);
                alert(jsonData);
    ...
    $("#btnRemove").click(function () {
        ...
        if (CheckBoxOnly("chkSelect")) {
            if (confirm("确定要删除该信息吗?")) {
                ...
                InvokeAction.InvokeJsonAction('AdviceMgr', 'DeleteAdvice', param, function (jsonData) {
                    PageClick(1);
                    alert(jsonData);
                });
            }
